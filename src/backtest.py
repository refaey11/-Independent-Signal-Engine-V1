...
def run_backtest(df, warmup=60, fee_bps=5):
    """Backtest using only bars available at each decision timestamp.

    Higher-timeframe frames are rebuilt from the causal history at each bar, so
    future candles cannot influence the current signal.
    """
